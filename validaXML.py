...
try:
    ...
    print("Erro: biblioteca 'rich' nao encontrada. Execute: pip install rich")
    sys.exit(1)
...
def validate_xsd(xml_path: str, xsd_path: str) -> ValidationResult:
    ...
    if not XMLSCHEMA_AVAILABLE and not LXML_AVAILABLE:
        result.errors.append("Nenhuma biblioteca de validacao XSD disponivel. Instale: pip install xmlschema ou lxml")
        result.duration_ms = round((time.time() - start) * 1000, 2)
        return result
    ...
    try:
        if LXML_AVAILABLE:
            ...
            result.is_valid = xsd.validate(xml_doc)
...
def validate_xpath(xml_path: str, xpath_expressions: list) -> ValidationResult:
    ...
    if not LXML_AVAILABLE:
        result.errors.append("lxml necessario para validacao XPath. Instale: pip install lxml")
        result.duration_ms = round((time.time() - start) * 1000, 2)
        return result
    ...
    try:
        ...
        for expr in xpath_expressions:
            try:
                ...
                if count == 0:
                    result.warnings.append(f"XPath sem resultados: {expr}")
            except etree.XPathError as e:
                result.errors.append(f"Expressao XPath invalida '{expr}': {str(e)}")
...
def render_result(result: ValidationResult, show_info: bool = True):
    if result.is_valid:
        status_text = Text("VALIDO", style=f"bold {THEME['success']}")
        panel_border = THEME["success"]
    else:
        status_text = Text("INVALIDO", style=f"bold {THEME['error']}")
        ...
        for key, val in result.info.items():
            if key == "xpath_results":
                for xp, xr in val.items():
                    icon = "ok" if xr["ok"] else "nao encontrado"
                    color = THEME["success"] if xr["ok"] else THEME["warning"]
                    info_table.add_row("XPath", Text(f"[{icon}] {xp}  ({xr['found']} no(s))", style=color))
            else:
                info_table.add_row(key.replace("_", " ").title(), str(val))
    ...
    if result.errors:
        err_table = Table(
            title=Text(f"Erros ({len(result.errors)})", style=f"bold {THEME['error']}"),
            box=box.ROUNDED, border_style=THEME["error"], show_header=False, padding=(0, 1),
        )
        err_table.add_column("Descricao", style="white")
        for err in result.errors:
            err_table.add_row(Text(f"  {err}", style=THEME["error"]))

    if result.warnings:
        warn_table = Table(
            title=Text(f"Avisos ({len(result.warnings)})", style=f"bold {THEME['warning']}"),
            box=box.ROUNDED, border_style=THEME["warning"], show_header=False, padding=(0, 1),
        )
        warn_table.add_column("Descricao", style="white")
        for w in result.warnings:
            warn_table.add_row(Text(f"  {w}", style=THEME["warning"]))
    ...
    try:
        console.print(Panel(panel_content, title=header, border_style=panel_border, padding=(1, 2)))
...
def render_batch_summary(results: list):
    ...
    try:
        console.print()
        console.print(Rule(Text("RELATORIO DE LOTE", style=f"bold {THEME['primary']}"), style=THEME["primary"]))
        console.print()

        summary_table = Table(box=box.SIMPLE_HEAVY, show_header=False, padding=(1, 4), border_style=THEME["primary"])
        ...
        summary_table.add_row(
            Text(f"{total}", style=f"bold {THEME['primary']}"),
            Text(f"{valid}", style=f"bold {THEME['success']}"),
            Text(f"{invalid}", style=f"bold {THEME['error']}"),
            Text(f"{round(total_ms)}ms", style=f"bold {THEME['warning']}"),
        )
        summary_table.add_row(
        ...
        tbl = Table(
            ...
            box=box.ROUNDED, border_style=THEME["primary"],
            header_style=f"bold {THEME['primary']}", show_lines=False, padding=(0, 1),
        )
        ...
        for i, r in enumerate(results, 1):
            status = Text("OK", style=THEME["success"]) if r.is_valid else Text("ERRO", style=THEME["error"])
            ...
            tbl.add_row(str(i), r.filename, status, err_cell, warn_cell, f"{r.file_size_kb} KB", f"{r.duration_ms}ms")
        ...
        console.print(f"\n=== RELATORIO DE LOTE ===")
        console.print(f"Total: {total} | Validos: {valid} | Invalidos: {invalid} | Taxa: {pct:.1f}%")
        for i, r in enumerate(results, 1):
            console.print(f"{i}. {r.filename} - {'OK' if r.is_valid else 'ERRO'}")
        console.print()
...
def save_report(results: list, output_path: str):
    ...
    console.print(f"\n[bold {THEME['success']}]Relatorio salvo em:[/] [underline white]{output_path}[/]")


def interactive_mode():
    console.print(Panel(
        Text.from_markup(
            f"[bold {THEME['primary']}]XML Validator Pro[/]\n\n"
            f"[{THEME['muted']}]Selecione o modo de validacao.[/]"
        ),
        box=box.ROUNDED, border_style=THEME["primary"], padding=(1, 3),
    ))

    console.print()
    options_table = Table(box=box.SIMPLE, show_header=False, padding=(0, 2))
    options_table.add_column("Opcao", style=f"bold {THEME['accent']}", width=6)
    options_table.add_column("Modo", style="bold white", width=30)
    options_table.add_column("Descricao", style=f"dim {THEME['muted']}")

    options_table.add_row("[1]", "Bem-Formado", "Verifica sintaxe XML basica")
    options_table.add_row("[2]", "Com XSD", "Valida contra um Schema XSD")
    options_table.add_row("[3]", "Com XPath", "Verifica expressoes XPath")
    options_table.add_row("[4]", "Lote", "Valida multiplos arquivos de uma pasta")
    ...
    while True:
        ...
        if choice == "0":
            ...
            if not os.path.exists(xml_path):
                console.print(f"[bold {THEME['error']}]Arquivo nao encontrado: {xml_path}[/]")
                continue
            with Progress(SpinnerColumn(style=THEME["primary"]), TextColumn("Validando..."), console=console) as p:
                p.add_task("", total=None)
                result = validate_wellformed(xml_path)
            render_result(result)
            if Confirm.ask(f"\n[{THEME['muted']}]Salvar relatorio JSON?[/]", default=False):
                out = Prompt.ask("Caminho do relatorio", default="relatorio.json")
            ...
            for p, label in [(xml_path, "XML"), (xsd_path, "XSD")]:
                if not os.path.exists(p):
                    console.print(f"[bold {THEME['error']}]Arquivo {label} nao encontrado: {p}[/]")
                    break
            else:
                with Progress(SpinnerColumn(style=THEME["primary"]), TextColumn("Validando contra XSD..."), console=console) as pr:
            ...
            if not os.path.exists(xml_path):
                console.print(f"[bold {THEME['error']}]Arquivo nao encontrado: {xml_path}[/]")
                continue
            console.print(f"[{THEME['muted']}]Digite as expressoes XPath (linha vazia para terminar):[/]")
            expressions = []
            while True:
            ...
            if not expressions:
                console.print(f"[bold {THEME['warning']}]Nenhuma expressao fornecida.[/]")
                continue
            with Progress(SpinnerColumn(style=THEME["primary"]), TextColumn("Executando XPath..."), console=console) as pr:
                pr.add_task("", total=None)
            ...
            if not os.path.isdir(folder):
                console.print(f"[bold {THEME['error']}]Pasta nao encontrada: {folder}[/]")
                continue
            xml_files = list(Path(folder).glob("**/*.xml"))
            if not xml_files:
                console.print(f"[bold {THEME['warning']}]Nenhum XML encontrado em: {folder}[/]")
                continue
            console.print(f"\n[{THEME['success']}]{len(xml_files)} arquivo(s) encontrado(s)[/]")
            xsd_path = None
            if Confirm.ask(f"[{THEME['muted']}]Usar schema XSD?[/]", default=False):
                xsd_path = Prompt.ask(f"[bold {THEME['primary']}]Caminho do XSD[/]")
                if not os.path.exists(xsd_path):
                    console.print(f"[bold {THEME['error']}]XSD nao encontrado[/]")
                    continue
            output_path = None
            if Confirm.ask(f"[{THEME['muted']}]Salvar relatorio JSON?[/]", default=True):
                output_path = Prompt.ask("Caminho do relatorio", default="relatorio_lote.json")
            console.print()
            results = []
            with Progress(
                ...
                for xml_file in xml_files:
                    progress.update(task, description=f"{xml_file.name[:30]}...")
                    r = validate_xsd(str(xml_file), xsd_path) if xsd_path else validate_wellformed(str(xml_file))
                    results.append(r)
                    progress.advance(task)
            render_batch_summary(results)
            if Confirm.ask(f"[{THEME['muted']}]Ver detalhes por arquivo?[/]", default=False):
                for r in results:
        ...
        console.print()
        if not Confirm.ask(f"[{THEME['muted']}]Fazer outra validacao?[/]", default=True):
            console.print()
            break
...
Exemplos:
  ...
  python validaXML.py -f arquivo.xml --quiet
    ...
    parser.add_argument("--xsd", help="Schema XSD para validacao")
    parser.add_argument("--xpath", nargs="+", help="Expressoes XPath para verificar")
    parser.add_argument("--batch", help="Pasta para validacao em lote")
    parser.add_argument("-o", "--output", help="Arquivo de saida JSON")
    parser.add_argument("--quiet", action="store_true", help="Saida minima (apenas exit code)")
    parser.add_argument("--no-banner", action="store_true", help="Omite cabecalho")
    ...
    if args.batch:
        ...
        if not folder.is_dir():
            console.print(f"[bold {THEME['error']}]Pasta nao encontrada: {args.batch}[/]")
            sys.exit(2)
        ...
        with Progress(
            ...
            console=console, disable=args.quiet,
        ) as progress:
        ...
        if not os.path.exists(args.file):
            console.print(f"[bold {THEME['error']}]Arquivo nao encontrado: {args.file}[/]")
            sys.exit(2)
        if args.xsd:
